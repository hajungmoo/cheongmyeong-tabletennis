...
test('관리자 토큰 없이 공지 저장·예약 취소·사진 조회·초대를 할 수 없다',async()=>{
 for(const action of ['bootstrap','dashboard','save','cancel','receipts','invite','revoke','adminImage']){const result=await request(action);assert.equal(result.status,401,action);assert.match(result.body.error,/로그인/);}
});
