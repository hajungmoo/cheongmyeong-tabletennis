...
function renderDashboard(){
 ...
 $('outboxList').querySelectorAll('[data-delete]').forEach(b=>b.addEventListener('click',()=>deleteNotice(b.dataset.delete)));
...
async function deleteNotice(id){if(busy)return;const n=notices.find(n=>n.id===id);if(!n)return;const message='“'+n.title+'” 공지를 삭제할까요?\n\n삭제하면 선수 화면에서도 사라집니다. 이미 휴대폰에 도착한 푸시 알림은 회수되지 않습니다.';if(!confirm(message))return;lock(true);try{if(isDemo){notices=notices.filter(item=>item.id!==id);demoImages.delete(id);}else await call('delete',{id});if(editingId===id)resetForm();await refresh();toast(isDemo?'공지 삭제 미리보기입니다.':'공지를 삭제했습니다.');}catch(error){toast(error.message,true);}finally{lock(false);}}
